...
public static class OutputGenerator
{
    ...
    public static string Export(IEnumerable<Movie> movies, string outputPath, Format format)
    {
        ArgumentNullException.ThrowIfNull(movies);
        ArgumentException.ThrowIfNullOrWhiteSpace(outputPath);

        var normalizedMovies = NormalizeMovies(movies);
        return ExportNormalizedMovies(normalizedMovies, outputPath, format, DateTimeOffset.UtcNow);
    }

    public static string Export(IEnumerable<Movie> movies, string outputPath)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(outputPath);
        return Export(movies, outputPath, ParseFormat(Path.GetExtension(outputPath)));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.Run(() =>
        {
            ...
            return ExportNormalizedMovies(normalizedMovies, outputPath, format, exportedAtUtc);
        }, cancellationToken);
    }
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(outputPath);
        return ExportAsync(movies, outputPath, ParseFormat(Path.GetExtension(outputPath)), cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var exportTasks = requestedFormats
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                var outputPath = BuildMultiExportPath(normalizedOutputPathBase, format);
                var resolvedOutputPath = ExportNormalizedMovies(normalizedMovies, outputPath, format, exportedAtUtc);
    ...
    private static Movie[] NormalizeMovies(IEnumerable<Movie> movies)
    {
        return movies
    ...
    private static string ExportNormalizedMovies(
        IReadOnlyCollection<Movie> normalizedMovies,
        string outputPath,
        Format format,
        DateTimeOffset exportedAtUtc)
    {
        ...
        {
            case Format.TXT:
                File.WriteAllText(resolvedOutputPath, BuildTxt(normalizedMovies, exportedAtUtc), Encoding.UTF8);
                break;
            case Format.CSV:
                File.WriteAllText(resolvedOutputPath, BuildCsv(normalizedMovies), Encoding.UTF8);
                break;
            case Format.JSON:
                File.WriteAllBytes(resolvedOutputPath, BuildJson(normalizedMovies, exportedAtUtc));
                break;
            case Format.XLSX:
                BuildXlsx(normalizedMovies, resolvedOutputPath);
                break;
            default:
    ...
    private static string BuildMultiExportPath(string outputPathBase, Format format)
    {
        return outputPathBase + GetDefaultExtension(format);
    }

    private static string BuildTxt(IReadOnlyCollection<Movie> movies, DateTimeOffset exportedAtUtc)
    {
        var rows = movies
        ...
        builder.AppendLine("Movie Report");
        builder.AppendLine($"Exported At (UTC): {exportedAtUtc:O}");
        builder.AppendLine($"Movie Count: {movies.Count.ToString(CultureInfo.InvariantCulture)}");
        builder.AppendLine();
    ...
    private static string BuildCsv(IReadOnlyCollection<Movie> movies)
    {
        var builder = new StringBuilder();
        ...
        {
            builder.AppendLine(string.Join(",", BuildBooleanResolutionRow(movie).Select(EscapeCsv)));
        }

        return builder.ToString();
    }

    private static byte[] BuildJson(IReadOnlyCollection<Movie> movies, DateTimeOffset exportedAtUtc)
    {
        var exportModel = new
        {
            exportedAtUtc,
            movieCount = movies.Count,
            movies = movies.Select(movie => new
            {
                name = movie.Name,
                year = movie.Year,
                imdbId = movie.ImdbId,
                tmdbId = movie.TmdbId,
                resolutions = movie.Resolutions.Select(FormatResolution).ToArray()
            })
        };
        ...
        return JsonSerializer.SerializeToUtf8Bytes(exportModel, options);
    }

    private static void BuildXlsx(IReadOnlyCollection<Movie> movies, string outputPath)
    {
        ...
        WriteZipEntry(archive, "xl/worksheets/sheet1.xml", BuildWorksheetXml(movies));
    }
...
""";
    ...
    private static string BuildWorksheetXml(IReadOnlyCollection<Movie> movies)
    {
        var headers = GetTabularHeaders();
        var rows = movies
        ...
        foreach (var row in rows)
        {
            ...
            for (var columnIndex = 0; columnIndex < row.Length; columnIndex++)
            {
                ...
                builder.AppendLine(columnIndex == 1 && !string.IsNullOrWhiteSpace(cellValue)
    ...
    private static string BuildDisplayTitle(Movie movie)
    {
        return movie.Year.HasValue
            ? $"{movie.Name} ({movie.Year.Value.ToString(CultureInfo.InvariantCulture)})"
            : movie.Name;
    }
    ...
    private static string[] BuildTextExportRow(Movie movie)
    {
        var availableResolutions = movie.Resolutions.ToHashSet();

        return
        [
            movie.Name,
            movie.Year?.ToString(CultureInfo.InvariantCulture) ?? string.Empty,
            movie.ImdbId ?? string.Empty,
            movie.TmdbId ?? string.Empty,
            .. ResolutionColumns.Select(resolution => FormatResolutionCheckmark(availableResolutions.Contains(resolution)))
        ];
    }

    private static string[] BuildBooleanResolutionRow(Movie movie)
    {
        var availableResolutions = movie.Resolutions.ToHashSet();
    ...
    {
        return string.Join(", ", resolutions.Select(FormatResolution));
    }
